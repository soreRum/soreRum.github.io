import { poll, timer } from './utils.js';
...
export function initDraftSync() {
  ...
  controls.innerHTML += `
    <div class="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow">
      ...
      <p class="mt-2 text-xs text-slate-600 dark:text-slate-300">Polls Sleeper every 10 seconds while active.</p>
    </div>
  `;
  ...
  document.getElementById('startBtn').addEventListener('click', async () => {
    ...
    if (!draftId) return alert('Please paste a valid Sleeper draft URL or numeric draft ID.');

    document.getElementById('startBtn').disabled = true;
    document.getElementById('stopBtn').disabled = false;

    await poll(draftId);
    timer.interval = setInterval(() => poll(draftId), 10000);
  });
